...
static char* collect_text(const __html_node__* node) {
    ...
    for (int i = 0; i < node->child_count; i++) {
        ...
        if (strcmp(ch->tag, "#text") == 0 &&
            ch->text_content && strlen(ch->text_content) > 0)
...
static void paint_node(__html_node__* node, __display_list__* dl) {
    if (!node || !node->style || !node->layout) return;
    if (strcmp(node->tag, "#text") == 0)         return;
    if (node->style->display == DISPLAY_NONE)    return;
    ...
    if (s->overflow_hidden) {
        ...
        clip.w = b->width; clip.h = b->height;
    ...
    if (has_bg || has_border || has_radius) {
        ...
        rect.w = b->width; rect.h = b->height;
    ...
    for (int i = 0; i < dl->count; i++) {
        ...
        case CMD_RECT:
            printf("[%3d] RECT      x=%.0f y=%.0f w=%.0f h=%.0f  "
                   ...
                   i, c->x, c->y, c->w, c->h,
                   c->fill_color.r, c->fill_color.g,
                   c->fill_color.b, c->fill_color.a,
                   c->border_width, c->border_radius);
            break;
        case CMD_TEXT:
        ...
        case CMD_IMAGE:
            printf("[%3d] IMAGE     x=%.0f y=%.0f w=%.0f h=%.0f  id=%u\n",
                   i, c->x, c->y, c->w, c->h, c->texture_id);
            break;
        }
    }
}
