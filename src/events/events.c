 ...
 *
 * Fixes vs original:
 *  1. event_key_up() was calling cb_keydown instead of cb_keyup.
 *  2. event_key_up() was using cb_userdata_keydown for the up event.
 *  3. fire() helper now takes explicit userdata pointer so each event
 *     uses its own slot.
 */
 ...
 * Helpers
 * ========================================================================= */
static void fire(__html_node__ *node,
...
static void mark_dirty(__html_node__ *node) {
    while (node) { node->is_dirty = true; node = node->parent; }
}
    ...
    if (now != es->hovered) {
        if (es->hovered) {
            ...
            fire(es->hovered, es->hovered->cb_hover,
                 es->hovered->cb_userdata_hover, x, y, 0, 0);
            flags |= EV_DIRTY;
        }
        if (now) {
            now->is_hovered = true;
...
int event_mouse_down(EventState *es, __html_node__ *root, int button) {
    ...
    if (node) {
        ...
        fire(node, node->cb_mousedown, node->cb_userdata_mousedown,
...
int event_mouse_up(EventState *es, __html_node__ *root, int button) {
    ...
    if (node) {
        ...
        fire(node, node->cb_mouseup, node->cb_userdata_mouseup,
             es->mouse_x, es->mouse_y, 0, 0);
        flags |= EV_CONSUMED;
        if (node==es->pressed) {
...
int event_quit(EventState *es) { (void)es; return EV_QUIT; }
