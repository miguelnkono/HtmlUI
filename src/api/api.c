 ...
 *
 * Fixes vs original:
 *  1. ui_reload(): set_owner_ui() now runs BEFORE run_pipeline() so that
 *     any mutation during the pipeline can find owner_ui.
 *  2. node_set_style(): replaces an existing property in the inline style
 *     string in-place instead of always appending. Prevents unbounded growth
 *     of the style attribute on repeated calls with the same property.
 *  3. node_on_mousedown / node_on_mouseup: store userdata in their own
 *     dedicated slots (cb_userdata_mousedown / cb_userdata_mouseup) rather
 *     than sharing cb_userdata_click.
 *  4. node_on_keyup: stores userdata in cb_userdata_keyup, not keydown.
 */
...
static void set_error(UI *ui, const char *msg) {
    if (ui) snprintf(ui->last_error, sizeof(ui->last_error), "%s", msg);
}

static void set_owner_ui(__html_node__ *node, UI *ui) {
    if (!node) return;
    node->owner_ui = ui;
    for (int i=0; i<node->child_count; i++) set_owner_ui(node->children[i], ui);
}
...
static void repaint(UI *ui) {
    ...
    if (ui->soft) {
        ...
        if (ui->dom_root && ui->dom_root->style) bg=ui->dom_root->style->background_color;
...
static int run_pipeline(UI *ui) {
    if (!ui->dom_root) return -1;
    cascade_apply(ui->dom_root, &ui->css_rules, (float)ui->window_w, (float)ui->window_h);
...
static void update_text_input(UI *ui) {
    ...
    if (focused && strcmp(focused->tag,"input")==0) SDL_StartTextInput(win);
...
static __html_node__ *query_one(__html_node__ *node, const char *sel) {
    if (!node) return NULL;
    if (strcmp(node->tag,"#text")!=0 && selector_matches(sel,node)) return node;
...
const char *htmlui_result_str(HtmluiResult r) {
    switch(r) {
    case HTMLUI_OK:             return "OK";
    case HTMLUI_ERR_IO:         return "IO error";
    case HTMLUI_ERR_PARSE_HTML: return "HTML parse error";
    case HTMLUI_ERR_PARSE_CSS:  return "CSS parse error";
    case HTMLUI_ERR_LAYOUT:     return "Layout error";
    case HTMLUI_ERR_RENDER_INIT:return "Renderer init error";
    case HTMLUI_ERR_NULL:       return "NULL pointer";
    case HTMLUI_ERR_NOT_FOUND:  return "Not found";
    case HTMLUI_ERR_OOM:        return "Out of memory";
 ...
 * ui_load
 * ========================================================================= */
UI *ui_load(const char *html_path, const char *css_path, const HtmluiOptions *opts) {
    UI *ui = calloc(1, sizeof(UI));
    if (!ui) return NULL;

    htmlui_options_default(&ui->options);
    if (opts) ui->options = *opts;
    ...
    if (html_path)
        ui->dom_root = html_parse_file(html_path, ui->last_error, sizeof(ui->last_error));
    else
        ui->dom_root = html_parse_string("<html><body></body></html>",
                                         ui->last_error, sizeof(ui->last_error));
    if (!ui->dom_root) { ui_destroy(ui); return NULL; }

    if (css_path) {
        if (css_parse_file(css_path, &ui->css_rules, ui->last_error,
                           sizeof(ui->last_error)) != 0) {
            ui_destroy(ui); return NULL;
        }
    }

    ui->soft = soft_renderer_create(ui->window_w, ui->window_h);
    if (!ui->soft) { set_error(ui,"soft renderer OOM"); ui_destroy(ui); return NULL; }

#ifdef HTMLUI_SDL3
    if (ui->options.renderer == HTMLUI_RENDERER_SDL3) {
        ui->sdl3 = sdl3_renderer_create(ui->options.window_title,
                                         ui->window_w, ui->window_h,
                                         ui->options.resizable, ui->options.vsync);
        if (!ui->sdl3) {
...
HtmluiResult ui_reload(UI *ui) {
    if (!ui) return HTMLUI_ERR_NULL;

    htmlnode_free(ui->dom_root); ui->dom_root = NULL;
    ...
    if (ui->html_path) {
        ui->dom_root = html_parse_file(ui->html_path,
                                        ui->last_error, sizeof(ui->last_error));
        if (!ui->dom_root) return HTMLUI_ERR_PARSE_HTML;
    }
    if (ui->css_path) {
...
static void node_mark_dirty(Node *node) {
    if (!node) return;
    node->is_dirty = true;
    if (node->owner_ui) node->owner_ui->dirty = true;
}
...
void node_set_style(Node *node, const char *property, const char *value) {
    ...
    if (existing) {
        ...
        char tmp[4096]; snprintf(tmp, sizeof(tmp), "%s", existing);
        char *sp=NULL, *d=strtok_r(tmp,";",&sp);
        while (d) {
            /* Skip leading whitespace */
            while (*d==' ') d++;
            if (*d=='\0') { d=strtok_r(NULL,";",&sp); continue; }

            char *colon = strchr(d,':');
            if (colon) {
                *colon='\0';
                /* Trim the property token */
                char *pk=d; while (*pk==' ') pk++;
                char *pe=pk+strlen(pk)-1; while (pe>pk && *pe==' ') *pe--='\0';
...
void node_add_class(Node *node, const char *class_name) {
    ...
    if (!existing) {
        ...
        if (node_has_class(node,class_name)) return;
        char buf[1024]; snprintf(buf,sizeof(buf),"%s %s",existing,class_name);
...
void node_remove_class(Node *node, const char *class_name) {
    ...
    char *sp=NULL, *tok=strtok_r(tmp," ",&sp); int first=1;
    while (tok) {
...
void node_toggle_class(Node *node, const char *class_name) {
    if (!node||!class_name) return;
    if (node_has_class(node,class_name)) node_remove_class(node,class_name);
    else node_add_class(node,class_name);
}
bool node_has_class(Node *node, const char *class_name) {
    ...
    while (*p) {
        ...
        if ((size_t)(p-s)==nlen && strncmp(s,class_name,nlen)==0) return true;
    }
    return false;
}
    ...
    if (!visible) {
        if (!node->force_hidden) {
            ...
            if (node->style) {
                ...
                case DISPLAY_FLEX:         node->saved_display=strdup("flex"); break;
                case DISPLAY_INLINE:       node->saved_display=strdup("inline"); break;
                case DISPLAY_INLINE_BLOCK: node->saved_display=strdup("inline-block"); break;
                default:                   node->saved_display=strdup("block"); break;
                }
            }
        }
...
void node_on_click(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    n->cb_click = (void(*)(void*))cb; n->cb_userdata_click = ud;
}
void node_on_mousedown(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    /* FIX: was writing ud to cb_userdata_click, clobbering the click handler's data */
    n->cb_mousedown = (void(*)(void*))cb; n->cb_userdata_mousedown = ud;
}
void node_on_mouseup(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    /* FIX: same as above */
    n->cb_mouseup = (void(*)(void*))cb; n->cb_userdata_mouseup = ud;
}
void node_on_hover(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    n->cb_hover = (void(*)(void*))cb; n->cb_userdata_hover = ud;
}
void node_on_keydown(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    n->cb_keydown = (void(*)(void*))cb; n->cb_userdata_keydown = ud;
}
void node_on_keyup(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    /* FIX: was writing ud to cb_userdata_keydown */
    n->cb_keyup = (void(*)(void*))cb; n->cb_userdata_keyup = ud;
}
void node_on_change(Node *n, HtmluiEventCb cb, void *ud) {
    if (!n) return;
    n->cb_change = (void(*)(void*))cb; n->cb_userdata_change = ud;
}
        ...
        while (SDL_PollEvent(&e)) {
                ...
                if (e.key.scancode==SDL_SCANCODE_ESCAPE) {
                    ...
                    if (focused && strcmp(focused->tag,"input")==0) {
                        ...
                        if (current && strlen(current)>0) {
                            char buf[1024]; snprintf(buf,sizeof(buf),"%s",current);
                    ...
                    int flags=event_key_down(&ui->events,(int)e.key.scancode,(uint32_t)e.key.mod);
                    if (flags&EV_DIRTY) ui->dirty=true;
                }
                break;
            case SDL_EVENT_KEY_UP: {
                int flags=event_key_up(&ui->events,(int)e.key.scancode,(uint32_t)e.key.mod);
                if (flags&EV_DIRTY) ui->dirty=true;
                break;
            }
            case SDL_EVENT_MOUSE_MOTION: {
                int flags=event_mouse_move(&ui->events,ui->dom_root,e.motion.x,e.motion.y);
                if (flags&EV_DIRTY) ui->dirty=true;
                break;
            }
            case SDL_EVENT_MOUSE_BUTTON_DOWN: {
                int flags=event_mouse_down(&ui->events,ui->dom_root,e.button.button);
                if (flags&EV_DIRTY) ui->dirty=true;
            ...
            case SDL_EVENT_TEXT_INPUT: {
                ...
                if (focused && strcmp(focused->tag,"input")==0) {
                    const char *current=node_get_attr(focused,"value");
                    char buf[1024]={0};
                    if (current) snprintf(buf,sizeof(buf),"%s",current);
            ...
            case SDL_EVENT_WINDOW_RESIZED:
                ui->window_w=e.window.data1; ui->window_h=e.window.data2;
                if (ui->soft) soft_renderer_resize(ui->soft,ui->window_w,ui->window_h);
                cascade_apply(ui->dom_root,&ui->css_rules,(float)ui->window_w,(float)ui->window_h);
                ui->dirty=true;
                break;
            default: break;
...
void ui_render_frame(UI *ui) {
    if (!ui||!ui->running||!ui->dirty) return;
    cascade_apply(ui->dom_root,&ui->css_rules,(float)ui->window_w,(float)ui->window_h);
    layout_run(ui->dom_root,(float)ui->window_w,(float)ui->window_h);
    repaint(ui);
    ui->dirty=false;
}
void ui_invalidate(UI *ui) { if (ui) ui->dirty=true; }
...
void node_get_rect(Node *node, float *ox, float *oy, float *ow, float *oh) {
    ...
    if (ox) *ox=node->layout->x;
    if (oy) *oy=node->layout->y;
    if (ow) *ow=node->layout->width;
    if (oh) *oh=node->layout->height;
}
