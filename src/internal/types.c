...
// hex_nib for hexadecimal nibblle(4 bits)
...
__color__ color_parse(const char *str) {
  ...
  if (*str == '#') {
    ...
    if (len >= 6)
      return (__color__){hex2(str), hex2(str + 2), hex2(str + 4), 255};
    if (len >= 3) {
      char r2[3] = {str[0], str[0], '\0'}, g2[3] = {str[1], str[1], '\0'},
           b2[3] = {str[2], str[2], '\0'};
      return (__color__){hex2(r2), hex2(g2), hex2(b2), 255};
    }
  }
  if (strncmp(str, "rgb(", 4) == 0) {
    int r = 0, g = 0, b = 0;
    sscanf(str + 4, "%d , %d , %d", &r, &g, &b);
    return (__color__){(uint8_t)r, (uint8_t)g, (uint8_t)b, 255};
  }
  if (strncmp(str, "rgba(", 5) == 0) {
    int r = 0, g = 0, b = 0;
    float a = 1.f;
    sscanf(str + 5, "%d , %d , %d , %f", &r, &g, &b, &a);
    return (__color__){(uint8_t)r, (uint8_t)g, (uint8_t)b, (uint8_t)(a * 255)};
  }
  ...
  if (!strcmp(str, "white"))
    return COLOR_WHITE;
  if (!strcmp(str, "red"))
    return (__color__){255, 0, 0, 255};
  if (!strcmp(str, "green"))
    return (__color__){0, 128, 0, 255};
  if (!strcmp(str, "blue"))
    return (__color__){0, 0, 255, 255};
  if (!strcmp(str, "gray") || !strcmp(str, "grey"))
    return (__color__){128, 128, 128, 255};
  if (!strcmp(str, "yellow"))
    return (__color__){255, 255, 0, 255};
  if (!strcmp(str, "orange"))
    return (__color__){255, 165, 0, 255};
  if (!strcmp(str, "purple"))
    return (__color__){128, 0, 128, 255};
  if (!strcmp(str, "pink"))
    return (__color__){255, 192, 203, 255};
  if (!strcmp(str, "cyan"))
    return (__color__){0, 255, 255, 255};
  if (!strcmp(str, "magenta"))
    return (__color__){255, 0, 255, 255};
  return COLOR_TRANSPARENT;
}
...
void htmlnode_append_child(__html_node__ *parent, __html_node__ *child) {
  ...
  if (parent->child_count >= parent->child_capacity) {
    ...
    __html_node__ **ch =
        realloc(parent->children, sizeof(__html_node__ *) * (size_t)nc);
    if (!ch)
      return;
    parent->children = ch;
...
const char *htmlnode_id(const __html_node__ *n) {
  return htmlattr_list_get(&n->attrs, "id");
}

const char *htmlnode_class(const __html_node__ *n) {
  return htmlattr_list_get(&n->attrs, "class");
}

// __computed_style__ lifecycle
...
__computed_style__ computed_style_defaults(void) {
  ...
  s._color_set = false;
  s.font_size = NAN;
  s._font_size_set = false;
  s.font_family = NULL;
  s.line_height = NAN;
  s._line_height_set = false;
  s.font_weight = -1;
  s._font_weight_set = false;
  s.font_italic = -1;
  s._font_italic_set = false;
  s.text_align = -1;
  s._text_align_set = false;
...
void css_rule_list_add(__css_rule_list__ *list, __css_rule__ rule) {
  if (list->count >= list->capacity) {
    ...
    __css_rule__ *rules =
        realloc(list->rules, sizeof(__css_rule__) * (size_t)nc);
    if (!rules)
      return;
    list->rules = rules;
    list->capacity = nc;
