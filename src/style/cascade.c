 ...
 * Value resolution
 * ========================================================================= */
static float parse_length(const char *v, float parent_px, float fs,
                          float root_fs) {
  if (!v)
    return 0.f;
  ...
  if (strcmp(end, "px") == 0)
    return n;
  if (strcmp(end, "%") == 0)
    return isnan(parent_px) ? NAN : n * parent_px / 100.f;
  if (strcmp(end, "em") == 0)
    return n * (isnan(fs) ? 16.f : fs);
  if (strcmp(end, "rem") == 0)
    return n * root_fs;
  if (*end == '\0')
    return n;
...
static void parse_sh4(const char *v, float out[4], float pw, float fs,
                      float rfs) {
  if (!v)
    return;
  char buf[256];
  ...
  if (np == 1) {
    ...
    float v1 = parse_length(pp[1], pw, fs, rfs),
          v2 = parse_length(pp[2], pw, fs, rfs);
    out[0] = v0;
    out[1] = out[3] = v1;
    out[2] = v2;
  } else {
...
#define LH(v) parse_length((v), ph, fs, rfs)
  ...
  if (!strcmp(prop, "border-radius")) {
    float r = L(val);
    s->border_radius[0] = s->border_radius[1] = s->border_radius[2] =
        s->border_radius[3] = r;
    return;
  }
  ...
  if (!strcmp(prop, "border")) {
    ...
    while (tok) {
      if (isdigit((unsigned char)tok[0]) || tok[0] == '.') {
        ...
        s->border_width[0] = s->border_width[1] = s->border_width[2] =
            s->border_width[3] = w;
        ...
        s->border_color[0] = s->border_color[1] = s->border_color[2] =
            s->border_color[3] = c;
      }
  ...
  if (!strcmp(prop, "font-size")) {
    if (!strcmp(val, "small")) {
      ...
      s->font_size = parse_length(val, isnan(pfs) ? 16.f : pfs,
                                  isnan(pfs) ? 16.f : pfs, rfs);
    }
    s->_font_size_set = true;
    return;
  }
  ...
  if (!strcmp(prop, "font-weight")) {
    ...
    s->_font_weight_set = true;
    return;
  }
  if (!strcmp(prop, "font-style")) {
    s->font_italic =
        (!strcmp(val, "italic") || !strcmp(val, "oblique")) ? 1 : 0;
    s->_font_italic_set = true;
    return;
  }
  ...
  if (!strcmp(prop, "align-self")) {
    ...
    if (!strcmp(val, "auto"))
      s->align_self = ALIGN_SELF_AUTO;
    else if (!strcmp(val, "flex-start"))
      s->align_self = (int)ALIGN_FLEX_START;
    else if (!strcmp(val, "flex-end"))
      s->align_self = (int)ALIGN_FLEX_END;
    else if (!strcmp(val, "center"))
      s->align_self = (int)ALIGN_CENTER;
    else if (!strcmp(val, "stretch"))
      s->align_self = (int)ALIGN_STRETCH;
    return;
  }
  ...
  if (!strcmp(prop, "flex")) {
    ...
    if (ntok == 1) {
      shrink = 1.f;
      basis = 0.f;
    } /* flex:1 → basis=0, not auto */
    s->flex_grow = grow;
    s->flex_shrink = shrink;
    s->flex_basis = basis;
    return;
  }
  if (!strcmp(prop, "gap") || !strcmp(prop, "row-gap") ||
      !strcmp(prop, "column-gap")) {
    s->gap = L(val);
    return;
  }
 ...
 * ========================================================================= */
static void inherit_from_parent(__computed_style__ *s,
                                const __computed_style__ *p) {
  if (!p)
    return;
  ...
  /* font-weight */
  if (!s->_font_weight_set) {
    s->font_weight = (p->font_weight >= 0) ? p->font_weight : 400;
  }
  /* font-italic */
  if (!s->_font_italic_set) {
    s->font_italic = (p->font_italic >= 0) ? p->font_italic : 0;
  }
  /* line-height */
  if (!s->_line_height_set) {
    s->line_height = !isnan(p->line_height) ? p->line_height : 1.2f;
  }
  /* text-align */
  if (!s->_text_align_set) {
    s->text_align = (p->text_align >= 0) ? p->text_align : 0;
  }
}
 ...
 * ========================================================================= */
static void apply_inline_style(__computed_style__ *s, const char *inl, float pw,
                               float ph, float pfs, float rfs) {
  if (!inl)
    return;
  char buf[2048];
  strncpy(buf, inl, 2047);
  buf[2047] = '\0';
...
static void cascade_node(__html_node__ *node, const __css_rule_list__ *rules,
                         const __computed_style__ *ps, float vpw, float vph,
                         float rfs) {
  ...
  *s = computed_style_defaults();
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < r->decl_count; j++)
      apply_decl(s, r->declarations[j].property, r->declarations[j].value, pw,
                 ph, pfs, rfs);
  }
  free(matched);

  /* Inline style has highest priority (after !important, which we don't
   * support) */
